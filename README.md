...
## 🚀 はじめに
**重要**: インストール後の初回起動時は、**KonomiTV** もしくは **EDCB** および **Mirakurun（オプション）** のサーバー設定が必要です。
画面の指示に従って、バックエンドのアドレス情報（KonomiTVアドレス、IPアドレスやポート番号）を入力してください。
また、バックエンドにMirakurunを使用していない場合は、MirakurunのIPアドレスとポート番号の入力は不要です。

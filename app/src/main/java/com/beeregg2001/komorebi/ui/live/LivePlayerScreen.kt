...
fun LivePlayerScreen(
    ...
    // ★修正: 親から引数で貰っていたgroupedChannelsを、ここで直接監視する
    ...
    var currentQuality by remember(initialQuality) {
        mutableStateOf(
            StreamQuality.fromValue(
                initialQuality
            )
        )
    }
    ...
    fun analyzePlayerError(error: PlaybackException): String {
        ...
        return if (cause is HttpDataSource.InvalidResponseCodeException) {
            when (cause.responseCode) {
                404 -> AppStrings.ERR_CHANNEL_NOT_FOUND; 503 -> AppStrings.ERR_TUNER_FULL; else -> "サーバーエラー (HTTP ${cause.responseCode})"
            }
    ...
    val exoPlayer = remember(currentStreamSource, retryKey, currentQuality) {
        val renderersFactory = object : DefaultRenderersFactory(context) {
            override fun buildAudioSink(
                ctx: android.content.Context, enableFloat: Boolean, enableParams: Boolean
            ): DefaultAudioSink? {
            ...
            if (currentStreamSource == StreamSource.MIRAKURUN) {
                setMediaSourceFactory(
                    DefaultMediaSourceFactory(
                        tsDataSourceFactory, extractorsFactory
                    )
                )
            }
        }.build().apply {
    ...
    DisposableEffect(currentChannelItem.id, currentStreamSource, retryKey, currentQuality) {
        ...
        val eventUrl = UrlBuilder.getKonomiTvLiveEventsUrl(
            konomiIp, konomiPort, currentChannelItem.displayChannelId, currentQuality.value
        )
        ...
        val listener = object : EventSourceListener() {
            override fun onEvent(
                ...
                runCatching {
                    ...
                    if (sseStatus == "Error" || (sseStatus == "Offline" && (sseDetail.contains("失敗") || sseDetail.contains(
                            "エラー"
                        )))
                    ) {
                    ...
                    when (sseStatus) {
                        ...
                        "ONAir" -> {
                            if (!exoPlayer.isPlaying && playerError == null) exoPlayer.play()
                        }
                    }
                }
            }
    ...
    LaunchedEffect(exoPlayer, isSubtitleEnabled) {
        while (true) {
            if (isSubtitleEnabled && exoPlayer.isPlaying) {
                ...
                webViewRef.value?.post {
                    webViewRef.value?.evaluateJavascript(
                        "if(window.syncClock){ window.syncClock($currentPos); }", null
                    )
                }
            }
            delay(100)
    ...
    LaunchedEffect(currentChannelItem.id, retryKey) {
        ...
        delay(4500)
        if (!updatedIsManualOverlay && !updatedIsPinnedOverlay && !updatedIsSubMenuOpen) {
    ...
    Box(
                ...
                when (keyCode) {
                    ...
                    NativeKeyEvent.KEYCODE_DPAD_UP -> {
                        if (showOverlay && isManualOverlay) {
                            coroutineScope.launch { scrollState.animateScrollTo(scrollState.value - 200) }; return@onKeyEvent true
                        }
        ...
        AnimatedVisibility(visible = isPinnedOverlay && playerError == null) {
            StatusOverlay(
                currentChannelItem, mirakurunIp, mirakurunPort, konomiIp, konomiPort
            )
        }

        val displayTitle = currentChannelItem.programPresent?.title ?: "番組情報なし"
        AnimatedVisibility(
            visible = showOverlay && playerError == null && !isMiniListOpen,
        ...
        AnimatedVisibility(
            ...
            ChannelListOverlay(
                groupedChannels, currentChannelItem.id, {
        ...
        AnimatedVisibility(
            visible = isSubMenuOpen && playerError == null,
            enter = slideInVertically(initialOffsetY = { -it }) + fadeIn(),
            exit = slideOutVertically(targetOffsetY = { -it }) + fadeOut()
        ) {
            TopSubMenuUI(
                        ...
                        exoPlayer.trackSelectionParameters.buildUpon()
                            .clearOverridesOfType(C.TRACK_TYPE_AUDIO).addOverride(
                                TrackSelectionOverride(
                                    audioGroups[if (currentAudioMode == AudioMode.SUB) 1 else 0].mediaTrackGroup,
                                    0
                                )
                            ).build()
                ...
                },
                {
                    if (isMirakurunAvailable) {
                        currentStreamSource =
                            if (currentStreamSource == StreamSource.MIRAKURUN) StreamSource.KONOMITV else StreamSource.MIRAKURUN; onShowToast(
                            "ソース切替"
                ...
                },
                {
                    if (currentQuality != it) {
                        currentQuality = it; retryKey++; onShowToast("画質: ${it.label}")
                    }; onSubMenuToggle(false)
                },
                { onSubMenuToggle(false) })
        }

        if (playerError != null) LiveErrorDialog(
            errorMessage = playerError!!,
            onRetry = { playerError = null; retryKey++ },
                    ...
                    if (ownerString.contains("aribb24", true) || ownerString.contains(
                        ...
                        if (privateDataStart + privateDataLength <= rawData.size) {
                            val privateData = rawData.copyOfRange(
                                privateDataStart, privateDataStart + privateDataLength
                            )
                            val base64Data = Base64.encodeToString(privateData, Base64.NO_WRAP)
                            val currentPtsMs =
                                (timestampAdjuster.lastAdjustedTimestampUs / 1000) + LivePlayerConstants.SUBTITLE_SYNC_OFFSET_MS
                            android.util.Log.i(
                                TAG,
                                "=====> [SUBTITLE] Sending to WebView. Len: ${base64Data.length} <====="
                            )
                            webViewRef.value?.post {
                                webViewRef.value?.evaluateJavascript(
...
private class DirectSubtitlePayloadReaderFactory(
    ...
    override fun createPayloadReader(
        streamType: Int, esInfo: TsPayloadReader.EsInfo
    ): TsPayloadReader? {
